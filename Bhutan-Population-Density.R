...
bt_hex <-
  st_read("Data/kontur_population_BT_20231101.gpkg") %>%
  st_transform(3106)

# load population by administrative boundary
bt_admin <-
  st_read("Data/kontur_boundaries_BT_20230628.gpkg") %>%
  st_transform(3106)


# Checking 'name_en' column in bt_admin data frame
distinct_names <- bt_admin %>%
  distinct(name_en)
...
outfile <- glue::glue("Plots/Bhutan.png")
