...
mv_hex <-
  st_read("Data/kontur_population_MaldiVes_20231101.gpkg") %>%
  st_transform(3106)

# load population by administrative boundary
mv_admin <-
  st_read("Data/kontur_boundaries_MV_20230628.gpkg") %>%
  st_transform(3106)


# Checking 'name_en' column in mv_admin data frame
distinct_names <- mv_admin %>%
  distinct(name_en)
print(distinct_names)


# Creating MV Boundary
...
outfile <- glue::glue("Plots/MalDives_BuPu2.png")
