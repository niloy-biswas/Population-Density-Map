...
mm_hex <-
  st_read("Data/kontur_population_MM_20231101.gpkg") %>%
  st_transform(3106)

# load population by administrative boundary
mm_admin <-
  st_read("Data/kontur_boundaries_MM_20230628.gpkg") %>%
  st_transform(3106)


# Checking 'name_en' column in mm_admin data frame
distinct_names <- mm_admin %>%
  distinct(name_en)
...
outfile <- glue::glue("Plots/Mayanmar2.png")
